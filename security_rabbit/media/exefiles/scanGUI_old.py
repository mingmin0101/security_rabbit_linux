...
class RabbitClient:
    def __init__(self):
        ...
        self.dir_arr = ['','','','','','']   # 待掃描資料夾arr [homepath, programfiles, programfiles(x86), windir, systemdrive, (option)]
    ...
    def initGUI(self):
        ...
        var_option = tk.IntVar(value=2)   # value是預設值
    ...
    def run(self):
        ...
        with open(filepath) as file:
            ...
            json_dict["metainfo"]["mode"]=self.scanType
            json_dict["metainfo"]["dict"]=self.directoryEdit
    ...
    def removeFiles(self):
        removeList = ["srcore.zip","__main__.exe","sigcheck.exe","userdb_filter.txt", self.filename] #,"error.json","exeInfo.xlsx"]
...
def path_selection():
    ...
    self.dir_arr[0] = os.environ['HOMEDRIVE'] + os.environ['HOMEPATH'] if (var_homepath.get() == 1) else ''

    if (var_programfiles.get() == 1):
        if(platform.architecture()[0] == "32bit"):
            self.dir_arr[1] = os.environ["ProgramFiles"]
        elif(platform.architecture()[0] == "64bit"):
            self.dir_arr[1] = os.environ["ProgramFiles"]        # 放64位元的
            self.dir_arr[2] = os.environ["ProgramFiles(x86)"]   # 放32位元的
    else:
        self.dir_arr[1] = ''
        self.dir_arr[2] = ''

    self.dir_arr[3] = os.environ['WINDIR'] if (var_windir.get() == 1) else ''
    self.dir_arr[4] = os.environ['systemdrive'] if (var_systemdrive.get() == 1) else ''
    self.dir_arr[5] = filepath_var.get() if (var_other.get() == 1 and filepath_var.get() != '請選擇路徑') else ''
